...
values_gb %>% 
  ...
  geom_line(aes(x = x, y = (pred_RLS - 0.01)), color = "green") +
...
get_w_BLR <- function(my_values, target, alpha, beta){
  ...
  return(BLR_updating(phi_mat = phi_mat, target = t_mat, alpha = alpha, beta = beta))
}
...
p <- ggplot(values_predictions, aes(x = x, y = preds)) +
  ...
  geom_line(aes(x = x, y = sin(2 * pi * x)), alpha = 0.4)
...
for (i in unique(values_gaussbases$n)) {
  ...
  phi_mat <- val_temp %>% 
    ...
    as.matrix()
  t_mat <- as.matrix(val_temp$target)
  coefs <- BLR(phi_mat = phi_mat, 
               target = t_mat, alpha = 0.5, beta = 10)
...
BLR_optimal <- function(phi_mat, t_mat, 
                        ...
                        min_d_alpha = 0.001, 
                        min_d_beta = 0.001, 
                        echo = FALSE, mean_prior = 2){
  alpha <- 0.5
  beta <- 10
  m_N <- as.matrix(rep(mean_prior, 9))
  S_N <- alpha * diag(1, 9)
  iteration <- 1 
  d_alpha <- 10
  d_beta <- 10
...
for (i in 1:5) {
  create_test_data(n = (20 * i), s_sq = 0.02)
...
for (i in unique(values_gaussbases$n)) {
  ...
  phi_mat <- val_temp %>% 
    ...
    as.matrix()
  t_mat <- as.matrix(val_temp$target)
  coefs <- BLR_optimal(phi_mat, t_mat)
