...
values_gb %>% 
  ...
  scale_color_viridis(discrete = TRUE, option = "inferno") + my_theme
```
...
w_BLRupd_result
```
We can observe that both of these methods provide us with identical weights. Therefore I will use easier method without updating afterwards.
...
plot_interactive_graph_BLS_RLS <- function(s_squared, 
  ...
  p <- ggplot(values_predictions, aes(x = x))  +
    ...
    my_theme + labs(y = "predictions and targets")
  plot <- ggplotly(p) %>%
